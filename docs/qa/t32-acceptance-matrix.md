...
| `RELEASE-SIGN` | `PASS` | Approved personal-use package identity and local signing input | Personal-use release APK | Approved identity, zip alignment, local signature verification and recorded SHA-256 | `T32-PERSONAL-RELEASE-20260820`, `docs/release/signing.md` | Personal-use signing only; this does not assert a public production release identity or publisher certificate | Clear signing environment; retain keystore only in user-private local storage |
