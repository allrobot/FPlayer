...
| `SMB-REAL` | `NOT_RUN` | Runtime-only authenticated source configuration | `FUNSCRIPT_TEST_LIBRARY` | Metadata scan, script match, disconnect, recovery and old-generation retention pass without secret logging | `docs/qa/t31-report.md`, `docs/superpowers/plans/2026-08-19-t32-device-media-acceptance.md` | Authorized authenticated source has not yet been executed in T32 | Close sessions, clear credential buffers, remove only app-owned cache and temporary rules |
